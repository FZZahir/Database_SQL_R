...
#time.series.xts=getRawFred(symbols=c('GDPC1','UNRATE','CPIAUCSL'),start_date='1979-09-01',collapse='quarterly',type='xts')
getRawFred<-function(symbols,...){
 # get Raw (not transformed) data from quandl
  usePackage('Quandl')
  #You must have a Quandl key stored in a local file
...
#gdp.fcst<-oneStepGDPGrowthForecast(time.series.xts=getRawFred(symbols=c('GDPC1','UNRATE','CPIAUCSL'),start_date='1979-09-01',collapse='quarterly',type='xts'))
oneStepGDPGrowthForecast<-function(time.series.xts,seed=1){
 initializeKeras() # Lexical error is a known April 2019 bug - workaround: in R: update tensorflow package; install devtools package; uninstall keras package; run: devtools::install_github("rstudio/reticulate",upgrade='never'); run: devtools::install_github("rstudio/keras",upgrade='never'); run: initializeKeras(first.time=T)
  ...
  #if the above causes problems, comment it and uncoment the line below
...
#Please do not use the link to my file, use your file or replace readChar() with your api key
path.to.private.key<-'C:/Users/PKalczynski/Dropbox (CSU Fullerton)/My Teaching/ISDS570/quandl.api.txt'

#load
symbols=c('GDPC1','UNRATE','CPIAUCSL')
